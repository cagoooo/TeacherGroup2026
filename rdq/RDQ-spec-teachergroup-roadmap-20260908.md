...
- 使用統計 → 保留瀏覽器本機 localStorage 匿名明細；中央端只接收固定事件名稱與網站版本，彙總至學校管理帳號持有的私人 Google 試算表，不建立跨裝置身分或去重教師人數。
...
- 不建立會員個資後台、第三方登入、付款功能或可辨識個人的分析資料庫；GAS 僅限固定事件的匿名每日彙總。

## 一段式需求規格
請以 **TeacherGroup2026** 現有靜態網站、**2026.09.08-4** 公開版本、**1,200＋300＝1,500 元**收費規則與石門國小校內分工為基礎，完成已選十項 Roadmap 的第一階段與已確認的匿名統計延伸；網站仍不得公開會員名冊、不得引導一般會員個別匯款，QR 只能指向公開頁面，中央統計只能接收固定事件名稱與網站版本，正式品牌素材則須有授權紀錄。網站需讓未來活動與研習更新可追溯、可驗證、可回復；會員私有後台、登入、付款與個資資料庫仍須另行確認資料權限與驗收條件。
...
- [x] `usage-stats.html` 保留本機匿名統計；中央端只以 `sendBeacon`／非阻塞 `fetch` 傳送固定事件與網站版本，未使用 Cookie、XHR 或外部分析服務。
- [x] `brand-assets.json` 明確記錄目前為 provisional site brand，未宣稱正式工會 Logo／QR／海報已取得授權。
- [x] GAS Web App health 與匿名事件 POST 均回應 HTTP 200，`initializeBackend` 已建立私人 Google 試算表，並完成前端 `/exec` 接線。
