...
### 後端後續改良候選（RDQ 草案，尚待選定）

現行匿名 GAS＋Google Sheets 後端已完成第一階段；以下候選不代表已授權開發，須依 `rdq/RDQ-spec-teachergroup-backend-roadmap-20260908.md` 完成選擇與確認。

| 編號 | 優先級／狀態 | 目標與使用情境 | 資料／權限風險 | 維護代價 | 建議驗收方式 |
| --- | --- | --- | --- | --- | --- |
| BACKEND-01 | P1／候選 | 維運健康檢查、每日備份、保存期限與復原演練；讓會長知道 GAS、試算表與前端接線是否正常。 | 中；備份檔仍可能含統計資料，必須維持私人權限並記錄保存期限。 | 低至中；需定期檢查觸發器、備份位置與失敗通知。 | 故意模擬端點失敗後可發現、復原；備份檔可讀且不公開；保留／刪除測試有紀錄。 |
| BACKEND-02 | P2／候選 | 私人匿名報表頁、日期篩選、事件排行與 CSV／JSON 匯出，協助判斷會員、活動與研習宣導入口的使用趨勢。 | 中；只顯示彙總資料，不得展示姓名或推論個別教師；匯出檔需受權限保護。 | 中；需維護查詢、時區、圖表與匯出格式。 | 以測試日期與事件資料驗證篩選、排序、匯出欄位；明確顯示「事件次數不等於教師人數」。 |
| BACKEND-03 | P2／候選 | 強化匿名端點的事件白名單、版本治理、錯誤監測與異常灌量提示，避免統計被大量重複請求扭曲。 | 中；完全匿名時無法保證一人一次，告警不可建立追蹤識別碼。 | 中；需設定門檻、檢視錯誤與人工覆核規則。 | 非白名單事件被拒絕；格式錯誤不寫入；異常量可被標示；不新增 IP、User-Agent 或 Cookie 收集。 |
| BACKEND-04 | P1／候選 | 管理者登入、角色權限與操作稽核；將會長、財務長、支會長的報表／設定可見範圍分開。 | 高；帳號異動、權限誤配與稽核紀錄均涉及管理責任。 | 中至高；需登入、權限矩陣、離職停權與稽核維護。 | 以測試帳號驗證最小權限、拒絕越權、停權與操作紀錄；不使用真實名冊測試。 |
...
- 後端後續規劃草案：`rdq/RDQ-spec-teachergroup-backend-roadmap-20260908.md`；目前狀態為 `draft`，待會長確認使用者、保存方式、會員後台範圍與優先順序後，才進入下一階段開發。
